...
#            custom_print_in_blue('block', block)
#            custom_print_in_blue('chunk', chunk)
            # Signals

#            custom_print_in_blue('packet_size', packet_size)
#            custom_print_in_blue('resolutions[np.newaxis, :]', resolutions[np.newaxis, :])
#            custom_print_in_blue('chunk[0]', chunk[0])

            integer, decimal = divmod(chunk[0][0], 1)

#            custom_print_in_blue('integer, decimal', integer, decimal)

            chunk *= resolutions[np.newaxis, :] #TODO according to chunk data format, apply resolution factor.
...
class BrainvisionSocket(DeviceBase):
    ...
    def initialize(self, streamhandler = None):
        ...
        buf_header = recv_data(brain_socket, 24)
        (id1, id2, id3, id4, msgsize, msgtype) = struct.unpack('<llllLL', buf_header)
#        custom_print_in_blue(self, '(id1, id2, id3, id4, msgsize, msgtype)', (id1, id2, id3, id4, msgsize, msgtype))
        rawdata = recv_data(brain_socket,  msgsize - 24)
        if msgtype == 1:
