...
class EmotivNonBlocking(DeviceBase, QtCore.QObject):

    signal_has_timed_out = QtCore.pyqtSignal(bool)
    ...
    def initialize(self):
        ...
        #init timer attribute
        self.timeout_timer = QtCore.QTimer()
        self.timeout_timer.setInterval(6000) #ms
        self.timeout_timer.timeout.connect(self.check_timeout_flag)

    @QtCore.pyqtSlot()
    def check_timeout_flag(self):
#        print self, 'self.last_data_received_timestamp.value', self.last_data_received_timestamp.value
        if abs(self.last_data_received_timestamp.value - time.time()) > 3.: #current timestamp in sec
            print self, 'has timeout'
            self.signal_has_timed_out.emit(True)

    def start(self):
        
        self.stop_flag = mp.Value('i', 0) #flag pultiproc  = global 
        self.last_data_received_timestamp = mp.Value('d', time.time())
        self.process = mp.Process(
                                target=emotiv_mainLoop, 
                                args=(self.stop_flag, self.streams, self.device_path, self.device_info['serial'], self.last_data_received_timestamp))
#        self.process = threading.Thread(group=None, target=emotiv_mainLoop, name=None, args=(self.stop_flag, self.streams, self.device_path, self.device_info['serial']), kwargs={})
        self.process.start()
...
#    hidraw = open(device_path)
    hidraw = os.open(device_path, os.O_RDONLY | os.O_NONBLOCK)
    ...
    while True:
#        print mp.current_process(), 'reading'
#        crypted_data = hidraw.read(32)
        try:
            crypted_data = os.read(hidraw, 32) #see non blocking flag in hidraw = os.open(..)
        except OSError: #OSError if nothing to read yet
            if stop_flag.value:
                print 'will stop'
                break
            else:
                time.sleep(.001) #this loop is not rhythmed by the emotiv hardware as os.read is non blocking
                continue

#        print mp.current_process(), 'just read'
        last_data_received_timestamp.value = time.time()
...
#        print mp.current_process(), 'sending', abs_pos
